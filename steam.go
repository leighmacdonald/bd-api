...
func profileUpdater(ctx context.Context, database *pgStore) {
	...
	for {
		select {
		case <-updateTicker.C:
			if err := performProfileUpdate(ctx, database); err != nil {
				slog.Error("Failed to update profiles", ErrAttr(err))
			}
		case <-ctx.Done():
			return
		}
...
func performProfileUpdate(ctx context.Context, database *pgStore) error {
	var (
